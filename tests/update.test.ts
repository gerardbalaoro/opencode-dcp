...
import { isAutoUpdatableSpec, isVersionNewer, updateRemoveDir } from "../src/lib/update"
...
test("updateRemoveDir removes opencode npm wrapper for latest installs", async () => {
    ...
    const wrapperDir = join(rootDir, "@gblab", "opencode-dcp@latest")
    const packageDir = join(wrapperDir, "node_modules", "@gblab", "opencode-dcp")
    await writePackageJson(wrapperDir, {
        dependencies: { "@gblab/opencode-dcp": "3.1.10" },
    })
    await writePackageJson(packageDir, {
        name: "@gblab/opencode-dcp",
        version: "3.1.9",
    })

    assert.equal(await updateRemoveDir(packageDir, "@gblab/opencode-dcp"), wrapperDir)
})

test("updateRemoveDir skips version-locked opencode installs", async () => {
    const rootDir = await mkdtemp(join(tmpdir(), "dcp-update-"))
    const wrapperDir = join(rootDir, "@gblab", "opencode-dcp@3.1.9")
    const packageDir = join(wrapperDir, "node_modules", "@gblab", "opencode-dcp")
    await writePackageJson(wrapperDir, {
        dependencies: { "@gblab/opencode-dcp": "3.1.9" },
    })
    await writePackageJson(packageDir, {
        name: "@gblab/opencode-dcp",
        version: "3.1.9",
    ...
    assert.equal(await updateRemoveDir(packageDir, "@gblab/opencode-dcp"), undefined)
})
